export const APP_URL_DEV = `http://piharwa.co.in/api/v1/`;

//  export const APP_URL_DEV = `http://localhost:3000/api/v1/`;

export const SIGNIN_URL = APP_URL_DEV + `user/login`;

export const ADD_PRODUCT = APP_URL_DEV + `product/add`;
export const GET_ALL_PRODUCTS = APP_URL_DEV + `seller/product/list`;
export const GET_SINGLE_PRODUCTS = APP_URL_DEV + `product/single-product/`;
export const UPDATE_SINGLE_PRODUCT = APP_URL_DEV + `product/update`;

export const GET_ALL_CATEGORIES = APP_URL_DEV + `category/list`;


// upload images Api's

export const ADD_PRODUCT_IMAGE = APP_URL_DEV + `product/upload/images/`;
